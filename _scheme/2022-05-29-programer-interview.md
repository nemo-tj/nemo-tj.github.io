...
## B站 - 直播推荐
- 2023-12-11
  + 二面：介绍营收相关的项目, ltv预估模型(累积分布概率 [P(i) - P(i-1)] * [T(i) - T(i-1)])
  + 

## 美团-到家效果广告
- 2023-12-11
  + 二面：精排ctr预估, 模型结构上的改进 Deep Context Interest Network for Click-Through Rate Prediction
...
- 2023-12-08 一面
  + 因果推断
  + 题目： 股票买卖使得收益最大, 一天内最多操作买或卖一次
    * 求取每个递增区间上的最小min 最大 max, profit = (max - min)


## 字节-搜索

- 2023-12-08 三面
  + 从一个无序的整数数组中寻找缺失的最小正整数
    * 将位置index利用起来, 标记对应位置为负数， x = nums[i], num[x] = -abs(nums[x])

- 2023-12-05 一面
  + 召回上的问题，正负样本选取、loss函数的设计, 双塔模型结构
  + 题目：给个一个正数数组nums，求最短连续子序列的长度，使得其求和大于等于target
    * 利用滑动窗口

## 字节-tiktok 本地生活
- 2023-11-29 二面
  + 给定两个数组 X, Y，其中的元素都是 > 0, 判断是否存在 x^y > y^x
  + 先取对数，再将判断条件变换成 log(x) / x > log(y) / y, 即可在线性复杂度解决
...
- 2023-11-08 二面
  + 二叉树中两个节点的公共父节点
  + 比较常规的问题, 整体介绍了下项目。有一个额外的收获是，手机厂商的手机数量正比于dau。高端手机市场，能够挣到硬件的钱。而在低端市场，手机本身不挣钱，而是靠后期的互联网模式变现。如应用商店、广告等
  + oppo vivo 小米 荣耀 等, 作为中低端手机市场的代表，规模不小，vivo 有2.8亿用户

## 字节-飞书搜索
- 2023-11-08 一面
  + 相对于指标压力，toB 业务更加注重用户体验，尤其是产品上极端情况的优化。是在保证基础体验的前提下，进行指标的优化
  + 旋转 递增数组，查找旋转点 => 二分查找，主要是判断是否在左侧 还是 右侧。

## 百度-推荐
- 2023-11-08 一面
  + 管理职责的划分, 2人通用优化, 3人业务优化; 通用优化相对要长期一些，而业务往往是相对产出明确的工作。人力分工上如此，但是有一些交叉借鉴的地方。
  
- 2023-11-12 二面
  + 收益归因
  + dpp mmr submodular 对比
